...
- ``preserve_user_sections: true`` -> one section span per marked section in
  the TRACKED src file (enumerated via
  :func:`setforge.migrations._frozen_markers.extract_sections` with
  ``allow_legacy=True``), each carrying ``capture_mode`` = the file's
...
def _translate_sections(tracked_file: CommentedMap, roots: MigrationRoots) -> bool:
    ...
    :func:`setforge.migrations._frozen_markers.extract_sections` (``allow_legacy=True``
