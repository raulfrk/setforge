...
def sanitize_controls(text: str) -> str:
    ...
    where ``X = chr(code - 0x40)``. Newline (``\\n``) is the sole exception —
    passed through unescaped so multi-line text keeps its line breaks. Every
    other character is returned as-is, so untrusted bytes can never drive the
    terminal.
    """
