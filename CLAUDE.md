...
- **Fast development loop:** `uv run python scripts/test_fast.py -q`. This runs the broad unit and temporary-filesystem integration suite with at most six xdist workers, work-stealing distribution, coverage disabled, and the deadline-free Hypothesis parallel profile. Extra pytest arguments pass through; use `-n N` before them to choose a smaller worker count. The current lane collects 4,235 tests; the immediately preceding 4,234-test snapshot completed in 3:44 on the six-core reference host. An earlier 4,177-test snapshot completed in 4:02 pytest / 4:13 wall, versus the serial baseline taking 1:50 to reach only 252 tests.
- **Coverage gate:** `workers=$(nproc); HYPOTHESIS_PROFILE=parallel uv run pytest -n "$(( workers < 6 ? workers : 6 ))" --dist=worksteal -m "not e2e_docker and not test_infra and not slow" --cov-fail-under=85`. The immediately preceding 4,234-test snapshot passed at 89.57% coverage in 5:30; an earlier pre-final-contract snapshot passed 4,178 tests at 89.25% in 7:33 pytest / 7:48 wall.
