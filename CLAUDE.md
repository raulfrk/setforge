...
A 233-test end-to-end suite at `tests/docker/` exercises `install`/`sync`/`compare`/`revert`/`validate`/`init`/`migrate`/`upgrade` and the new secrets-scan + reconcile-failure UX surfaces against a fresh Debian 12 container with real `claude`/`code`/`gitleaks` binaries — the canonical behavior-preservation gate for this project.
...
- **Warm CLI benchmark:** `uv run python scripts/benchmark_docker_cli.py --target smoke --repeats 20` reuses the content-addressed image, starts one container, discards three warm-ups, and reports raw samples plus median/mean/stdev/min/max and an approximate 95% mean interval for the real `docker exec … uv run setforge --version` boundary. One noisy 10-sample observation on the 2026-08-17 reference host moved from 4.390s median / 4.879s mean (σ 1.75s) before lazy UI imports to 4.148s / 4.450s (σ 1.02s) after; the overlapping uncertainty makes this directional evidence, not a timing guarantee.
- **Observable-boundary budget:** Docker is reserved for fresh-host, real-tool, TTY/POSIX, or cross-process persistence behavior. Portable formatting/model cases belong in unit tests and temporary-filesystem CLI workflows belong in integration. The first right-sizing pass reduced 253 total Docker tests / 10 PR-smoke tests to enforced ceilings of 233 total, 224 deterministic, 9 network-canary, and 9 PR-smoke tests; the exact smoke lane fell from 155.89s to 123.91s on the reference host. `scripts/check_e2e_manifest.py` fails if a lane grows without an explicit budget review.
...
- **Runtime with `-n 2` (default):** the pre-prune ~300-test suite took ~42 min on a 6-core host; reprofile the 224-test deterministic full lane before replacing this historical baseline.
...
The suite is gated by `pytest -m e2e_docker` and excluded from the default run. Process-level infrastructure checks (pytest/xdist hooks and abrupt-process recovery) use `test_infra` and are also excluded from the fast default. `pyproject.toml` sets `addopts = "-m 'not e2e_docker and not test_infra' ..."`; CI runs that lane separately with explicit branch coverage for its helper modules.

## Local Python tests

- **Fast development loop:** `uv run python scripts/test_fast.py -q`. This runs the broad unit and temporary-filesystem integration suite with at most six xdist workers, work-stealing distribution, coverage disabled, and the deadline-free Hypothesis parallel profile. Extra pytest arguments pass through; use `-n N` before them to choose a smaller worker count. The current marker lane collects exactly 4,647 tests. The immediately preceding 4,646-test snapshot completed in 5:10 pytest / 5:17 wall on the six-core reference host; the 4,558-test snapshot completed in 6:10 pytest / 6:18 wall; the 4,481-test snapshot was collection-only; the 4,472-test snapshot completed in 5:24 pytest (wall time was not recorded separately). The preceding 4,462-test snapshot completed in 5:15 pytest / 5:30 wall; the 4,451-test snapshot completed in 4:39 pytest / 4:47 wall; the 4,419-test snapshot completed in 5:11 pytest / 5:19 wall, and the 4,392-test snapshot completed in 5:33 pytest / 5:40 wall. These are load-sensitive observations, not timing guarantees or evidence that the added tests caused the difference. The preceding 4,358-test snapshot completed in 4:22 pytest / 4:29 wall while system load exceeded the six available CPUs; the 4,272-test snapshot completed in 3:38; the earlier 4,251-test snapshot completed in 4:16; the 4,234-test snapshot completed in 3:44; an older 4,177-test snapshot completed in 4:02 pytest / 4:13 wall, versus the serial baseline taking 1:50 to reach only 252 tests.
- **Coverage gate:** `workers=$(nproc); HYPOTHESIS_PROFILE=parallel uv run pytest -n "$(( workers < 6 ? workers : 6 ))" --dist=worksteal -m "not e2e_docker and not test_infra and not slow" --cov-fail-under=85`. The immediately preceding 4,646-test snapshot passed at 89.46% in 7:47 pytest / 8:04 wall. The preceding 4,558-test snapshot passed at 89.50% in 10:29 pytest / 10:54 wall; the 4,481-test snapshot was collection-only; the 4,472-test snapshot passed at 89.41% in 9:23 pytest (wall time was not recorded separately). The preceding 4,462-test snapshot passed at 89.56% in 7:52 pytest / 8:14 wall; the 4,451-test snapshot passed at 89.58% in 7:14 pytest / 7:32 wall; the 4,419-test snapshot passed at 89.65% in 6:46 pytest / 7:04 wall; the 4,392-test snapshot passed at 89.61% in 9:26 pytest / 9:42 wall, the 4,328-test snapshot at 89.63% in 8:11, and the 4,272-test snapshot at 89.63% in 6:02. Treat these as host-load-sensitive observations, not timing guarantees or causal performance comparisons. The 4,251-test snapshot passed at 89.31% in 7:24; the 4,234-test snapshot passed at 89.57% in 5:30; an earlier pre-final-contract snapshot passed 4,178 tests at 89.25% in 7:33 pytest / 7:48 wall.
- **Exhaustive-property gate:** `workers=$(nproc); HYPOTHESIS_PROFILE=parallel uv run pytest -n "$(( workers < 6 ? workers : 6 ))" --dist=worksteal -m "slow and not e2e_docker and not test_infra" --no-cov`. Run this alongside the coverage gate when resources permit. The current 20-test snapshot completed in 4:35 pytest / 4:40 wall; the preceding snapshots completed in 3:31 pytest / 3:37 wall, 3:49 pytest / 3:55 wall, 4:22 pytest / 4:28 wall, and 4:11 pytest / 4:17 wall. Coverage instrumentation made the final reconcile state machine exceed 13 minutes on the reference host while adding no behavioral rigor, so CI keeps this gate separate and runs both jobs concurrently. Its single explicit reconcile state-machine runner retains the prior strongest 100-example × 50-step exploration budget without also collecting Hypothesis's generated unittest alias.

The `slow` marker is a speed boundary, not a correctness waiver: it is excluded only by the explicit fast-local and coverage lanes. CI still requires the separate exhaustive-property job; Docker E2E and test-infrastructure checks remain separate explicit lanes.
