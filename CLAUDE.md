...
- **Fast development loop:** `uv run python scripts/test_fast.py -q`. This runs the broad unit and temporary-filesystem integration suite with at most six xdist workers, work-stealing distribution, coverage disabled, and the deadline-free Hypothesis parallel profile. Extra pytest arguments pass through; use `-n N` before them to choose a smaller worker count. A pre-final-contract snapshot on the six-core reference host ran 4,177 tests in 4:02 pytest / 4:13 wall, versus the serial baseline taking 1:50 to reach only 252 tests.
- **Coverage gate:** `workers=$(nproc); HYPOTHESIS_PROFILE=parallel uv run pytest -n "$(( workers < 6 ? workers : 6 ))" --dist=worksteal -m "not e2e_docker and not test_infra and not slow" --cov-fail-under=85`. A pre-final-contract snapshot passed 4,178 tests at 89.25% coverage in 7:33 pytest / 7:48 wall; the final lane collects 4,191 after adding the runner/workflow safety contracts.
