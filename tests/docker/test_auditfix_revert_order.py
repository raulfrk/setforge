...
first revert deletes the seeded base and restores the pre-install live
state, a SECOND revert must re-apply the install verbatim — the tracked
body redeployed and the merge base re-seeded byte-identically to the
first install.
...
def test_install_revert_revert_redoes_install(
    ...
    """install deploys + seeds base → revert undoes (deletes base, restores
    pre-install live) → SECOND revert REDOES the install verbatim.

    The redo leg is the gap: it must re-apply the install-applied state —
    the tracked body redeployed and the merge base re-seeded byte-identical
    to the first install — driven entirely by the reverse transition the
    first revert recorded. A mis-recorded reverse store snapshot would
    leave the live file absent (no redeploy) or the base absent.
    """
    c = docker_container()
    ...
    assert rc == 0, stderr
    assert not _exists(c, _LIVE_FORKED), "first revert must restore pre-install absence"
    ...
    rc, _stdout, stderr = _revert(c)
    assert rc == 0, stderr
    # Live is redeployed, byte-identical to the first install.
    assert c.read_text(_LIVE_FORKED) == _TRACKED_MD_BODY
    # The merge base is re-seeded byte-identical to the first install —
