...
3. **Deviation justifications** — per observation D, when the implementer's commit message claims "X was already true at <line>", verify that claim against the actual pre-branch state. Made-up justifications are IMPORTANT findings even when the change itself is sound.
